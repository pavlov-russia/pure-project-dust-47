...
const Header = () => {
  ...
  const handleCTAClick = () => {
    ...
    const offsetUp = isMobile ? -75 : -50; // смещение вниз для более низкой центровки
    ...
    if (heroEl && 'IntersectionObserver' in window) {
      const observer = new IntersectionObserver(
        (entries) => {
          ...
          const prev = ctaStateRef.current;
          let next = prev;
          // Гистерезис: включаем при < 15%, скрываем при > 35%
          if (prev) {
            if (entry.intersectionRatio > 0.35) next = false;
          } else {
            if (entry.intersectionRatio < 0.15) next = true;
          }
          if (next !== prev) {
            ctaStateRef.current = next;
            setShowCTA(next);
          }
        },
        {
    ...
    const handleScroll = () => {
      if (!ticking) {
        requestAnimationFrame(() => {
          ...
          let next = prev;
          if (prev) {
            if (scrollTop < 360) next = false;
          } else {
            if (scrollTop > 420) next = true;
          }
          if (next !== prev) {
            ctaStateRef.current = next;
            setShowCTA(next);
    ...
    <header className="fixed top-0 left-0 right-0 z-50 animate-fade-in">
      ...
      >
        ...
        <div
          ...
          <svg width="120" height="24" viewBox="0 0 257 51" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-[30px] md:h-8 w-auto drop-shadow-sm">
        ...
        <div className={`absolute right-3 md:right-6 bottom-[2px] transition-all duration-500 ease-out ${
          ...
          <Button 
            ...
            className="slow-pulse transition-all duration-300 !h-[30px] !min-h-[30px] !py-0 w-auto px-3 md:h-10 md:py-2 md:px-4 rounded-2xl"
            style={{
            ...
            icon={
              <svg width="20" height="20" viewBox="0 0 52 52" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-3 h-3 md:w-5 md:h-5">
          ...
          >
             <span className="hidden sm:inline btn text-xs">Получить индивидуальное КП</span>
             <span className="sm:hidden btn text-xs">Получить КП</span>
          </Button>
        </div>
        </div>
      </div>
